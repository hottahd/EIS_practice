...
# #### 引数の補足
#
# **`exp_set`（1 つのラスター位置で複数露出）**
#
# ふつうは 1 つのスリット位置で 1 回だけ露出しますが、**同じ位置で続けて
# 何回か露出する observing study** もあります（暗い輝線を稼ぐ、飽和を避けて
# 長短の露出を組み合わせる、時間変化を追う、など）。
# その場合データは 4 次元になり、`exp_set` でどれを使うか選びます
# （`'sum'` = 全部足して長時間露出とみなす、整数 = その露出だけ）。
# ヘッダの `nexp_prp` が 1 位置あたりの露出数です。
# **この観測は `nexp_prp = 1` なので関係ありません。**
#
# **sit-and-stare とは別物です。**
#
# | | スリット | データの並び |
# |---|---|---|
# | 普通のラスター | 1 ステップずつ動かす | x 軸 = 空間（と時間） |
# | **sit-and-stare** | **動かさない**。同じ場所を撮り続ける | x 軸 = **時間だけ** |
# | `nexp_prp > 1` | **動かしながら**、各位置で複数回露出 | 露出の軸が 1 本増えて 4 次元 |
#
# sit-and-stare は「場所を捨てて時間を取る」観測、
# `nexp_prp > 1` は「各位置で S/N を稼ぐ（または露出時間を変えて飽和を避ける）」
# ための仕組みです。
#
# **`abs_errs`（カウントの絶対値から誤差を出す）**
#
# 誤差は光子統計と読み出しノイズから作ります:
#
# $$ \sigma_{\rm counts} = \sqrt{N + \sigma_{\rm read}^2} $$
#
# $N$ が**その画素で数えた光子数**、$\sigma_{\rm read}$ が**読み出しノイズ**です。
#
# **読み出しノイズとは**: CCD に溜まった電荷を読み出して数値にするとき、
# 増幅器や AD 変換器が**それ自体でゆらぎを乗せます**。光が 1 つも来ていなくても、
# 読み出すたびに値が少しずつ違う。その大きさのことです。
# EIS ではシャッターを閉じて測ったばらつきが 2.29 DN で、
# ゲイン 6.3 e/DN を掛けて **14.4 電子**、光子に直して **195 Å で 0.83 個**。
#
# 光子統計 $\sqrt{N}$ とは出どころが違うので、**二乗和で足します**。
# 明るい画素では $\sqrt{N}$ が支配的ですが、**暗い画素では読み出しノイズが
# 誤差の下限**を決めます（数光子しか来ていない画素では効いてきます）。
#
# ところが level-1 のカウント $N$ は**暗電流を引いた後**なので、
# 暗い画素では**統計的なゆらぎで負になる**ことがあります。
# 負の数の平方根は取れません。そこで
#
# | | 負のカウントの扱い |
# |---|---|
# | `abs_errs=True`（既定） | $\sqrt{\|N\| + \sigma_{\rm read}^2}$ とする。**負の画素も生かす**。<br>欠損（−100 以下）だけをマスクする |
# | `abs_errs=False` | 負とゼロを**全部**マスクして誤差を 0 扱いにする |
#
# 既定が `True` なのは、**「負のカウント」は多くの場合エラーではなく
# 暗電流引き算のゆらぎ**だからです。捨てると暗い場所が系統的に消えます。
#
# **`count_offset`（カウントに定数を足してから較正する）**
#
# 誤差計算と較正の**前**に、全画素へ定数を足します。
# 上の「負のカウント」の扱いを試すためのつまみで、
# 例えば `count_offset=10` とすれば負のカウントがほぼ無くなり、
# `abs_errs=False` でも画素を捨てずに済みます。
# **暗い場所の扱いが結果にどれだけ効くかを調べる**とき以外は触りません。
#
# #### `cube.uncertainty.array` の中身
#
# 上の式そのものです。読み出しノイズは波長に依存します:
#
# - シャッターを閉じたときのばらつきから **2.29 DN**、ゲイン 6.3 e/DN で **14.4 電子**
# - 1 光子あたりの電子数は $(12398.5/\lambda)/3.65$（3.65 eV で 1 電子）
# - よって読み出しノイズ [光子] = 14.4 / その値
#
# 実測すると **195 Å で 0.83 光子、275 Å で 1.17 光子**です
# （長波長ほど 1 光子あたりの電子数が少ないので、光子に直すと大きくなる）。
#
# 較正を当てている場合（既定）は、カウントの誤差に同じ較正係数を掛けたものが入ります。
# **欠損画素の誤差はカウント単位で −100**（IDL の流儀）なので、
# 較正後は −100 × 較正係数（195 Å の窓では約 **−1072**）という大きな負の値になります。
# データ側の欠損フラグと同じ理屈です（付録 A）。
#
# #### `cube.wavelength` の波長補正について
#
# **観測時に何かをしているわけではありません。** 起きているのは物理現象です。
#
# EIS は 98 分で地球を回るあいだに日照と日陰を繰り返し、**装置の温度が変わって
# 分光器がわずかに伸び縮みします**。その結果、**同じ波長の光が当たる CCD 上の
# 位置が動きます**（1 軌道で 0.02 Å ほど＝ 195 Å で数十 km/s 相当）。
# さらにスリットは検出器に対してわずかに傾いているので、
# **スリットに沿った位置によっても波長がずれます**。
#
# eispac がやっているのは、**読み込むときに波長軸を差し引きで直す**ことだけです:
#
# ```python
# corrected_wave[i,j,:] = meta['wave'] - meta['wave_corr'][i,j]
# ```
#
# `wave_corr` は HDF5 のヘッダに入っている補正量（NRL が作ったもの）で、
# **データそのものは動かしません**。動かすのは波長のラベルの方です。
# 自分で level-0 から処理する場合は、この工程を自前でやる必要があります。
#
# **動く量はどれくらいか**（この観測での実測）:
#
# | | |
# |---|---|
# | 波長の動き（全体） | 0.0345 Å = **1.5 点ぶん** |
# | スペクトルウィンドウの幅 | 0.513 Å = 23 点（狭いもので） |
# | 割合 | **ウィンドウ幅の 7%** |
#
# つまり**輝線がウィンドウから出てしまう心配はありません**。
# ウィンドウは輝線が中心に来るように設計されており（この観測では
# Fe XII 195.119 が窓の中心から 0.001 Å）、
# ドリフトの数倍の余裕があります。
#
# **問題になるのは別のところです。** 0.0345 Å は 195 Å では **53 km/s** に相当し、
# **測りたいドップラー速度（±15 km/s 程度）より大きい**。
# 「輝線が見えるかどうか」ではなく「**速度が測れるかどうか**」に効きます（第 3 章）。
...
# ### 4. テンプレートを選ぶ
#
# **テンプレートとは「フィットの設計図」です。**
#
# スペクトルにガウシアンを当てるには、当てる前に決めておくことがあります。
#
# - **ガウシアンを何本立てるか**
# - それぞれの**中心波長はどのあたりか**（初期値と動かしてよい範囲）
# - **背景**を何次式で引くか
# - 成分どうしを**縛るか**（例: 弱い線の位置を強い線から 0.06 Å 離れた所に固定する）
#
# EIS のスペクトルウィンドウには、**目的の輝線だけが入っているとは限りません**。
# 近くの別の輝線が一緒に入っていることが多く、そのときは
# 「1 本のガウシアン + 背景」では合いません。
#
# この設計図を輝線ごとに用意したものがテンプレートで、
# **eispac は EIS でよく使う輝線について 119 個を同梱**しています。
# だから普通は自分で書かず、**測りたい輝線に合うものを選びます**。
#
# ファイル名は `イオン_波長_成分数` です:
#
#     fe_12_195_119.2c   →  Fe XII 195.119 を 2 成分で
#     ca_14_193_874.6c   →  Ca XIV 193.874 を 6 成分で
#
# **同じ波長域に複数のテンプレートがあります。**
# 何を主役にするか、どこまで丁寧に分離するかで選び分けます。

# %%
import eispac

for n in ["ca_14_193_874.2c", "ca_14_193_874.6c",
          "fe_11_192_813.2c", "ca_17_192_858.1c"]:
    t = eispac.read_template(eispac.data.get_fit_template_filepath(n + ".template.h5"))
    print(f"{n:20s} {[str(x) for x in t.template['line_ids']]}")

# %% [markdown]
# 読み方:
#
# - `ca_14_193_874.2c` と `.6c` は**同じ Ca XIV を測る**もの。
#   `.6c` は隣の弱い線まで立てて丁寧に分離します（そのぶん不安定になりやすい）
# - `fe_11_192_813.2c` と `ca_17_192_858.1c` は**同じ波長域なのに中身が違う**。
#   前者は Fe XI と O V を分離し、**Ca XVII を立てていません**。
#   後者は Ca XVII を 1 本だけ立てて、ブレンドを無視しています
#   → だから Ca XVII が論文の 5 倍になります（付録 G）
#
# **合うものが無ければ自作します。** その方法も付録 G にあります。
#
# ```python

...
# ## 1-3. 輝線ごとの観測データ
#
# 各画素のスペクトルを**波長方向に足し合わせる**と、輻射強度に近い値になります
# （山の面積を数えているので）。フィットしないぶん数秒で済む簡易版です。
# 連続光や隣の輝線も一緒に足してしまうので正確ではありませんが、
# 「どこに何があるか」を掴むには十分です。
#
# これを全画素についてやれば、輻射強度のマップになります。
# **形成温度の順**に 8 枚並べます。

...
**配るのは `EIS_workshop.ipynb` の 1 冊だけです。** モジュール 0〜7 が全部入っています。
...
| 1 | EIS のデータを見る（**欠損値の罠**） | 40 分 |
| 2 | スペクトル線フィット（成分順の罠、平均→fit の順番、誤差の床） | 50 分 |
| 3 | AIA 94 → Fe XVIII（**Eq.(A1) の誤植**） | 50 分 |
| 4 | 座標合わせと inter-moss 箱の選択 | 40 分 |
| 5 | **論文 Table 2 と答え合わせ**（山場） | 30 分 |
| 6 | 寄与関数と EM loci（**1/(4π) の罠**） | 40 分 |
| 7 | DEM インバージョン（ill-posed、手法依存性） | 60 分 |

**半日なら 5 まで、1 日なら 7 まで**進めます。途中で止めるだけなので、
コースごとに別のノートを配る必要はありません。
...
ソースは**モジュールごとに** `src/NN_*.py` に `# %%` 区切りの素の Python で書き、
...
生成時に、各モジュール先頭の「インストール」「ブートストラップ」のセルは
**先頭の 1 組だけ**にまとめられる。
...
**★ 引数なしの実行を必ず 1 回は通すこと。** 通し版は全モジュールが
**1 つの名前空間**で動くので、モジュール間の変数名の衝突は
こちらでしか見つからない。
...
観測データはその場で取得し（既にあれば何もしない）、
モジュール間の受け渡し（`work/box_intensities.csv`、
`data/cache/aia_on_eis_grid.npz`）は無ければ

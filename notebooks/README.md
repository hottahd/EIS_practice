...
**ノートに出る文章はすべて `src/*.py` の中にあります。**
`build_notebooks.py` は文章を持ちません（持たせると編集する人が探せなくなるため）。
冒頭の見出し・準備の説明・インストールのセルは `src/00_setup.py` にあります。
...
### 解析は `src/*.py` に書く（`scripts/` に畳まない）

フィットの仕方や平均の取り方は**教える中身そのもの**なので、
ノートブックに見える形で書く。`scripts/workshop.py` に置くのは
**データ取得と定数だけ**。import する行にはコメントを付ける。
